  ...
  describe("noncurried function", () => {
    it("should return 20 when 5 is given", done => {
      fetchAndInstantiate("/base/test/noncurried_function.wasm")
        .then(instance => {
          // Object.keys(instance.exports).forEach(f => console.info(f));

          var i8 = new Uint8Array(instance.exports.memory.buffer);
          var i32 = new Uint32Array(instance.exports.memory.buffer);

          // expect(i8[1024]).to.equal(0);
          expect(i32[1572 / 4]).to.equal(1);
          instance.exports._start();
          // expect(i8[1024]).to.equal(1);

          // expect(i32[1024]).to.equal(0);
          // var header = new Uint32Array(i8.slice(1552 / 4, (1552 + 80) / 4));
          // var fp1 = new Uint32Array(i8.slice(1024, 1024 + 80));
          // expect(i8[0]).to.equal(0);
          expect(i32[1568 / 4]).to.equal(1792);
          expect(i32[1572 / 4]).to.equal(1588);
          // expect(i32[1024 + 1572 / 4]).to.equal(1);

          // expect(new Uint32Array(i8.slice(1576, 1580))[0]).to.equal(1);
          // 158l4;
          expect(i32[1584 / 4]).to.equal(3063);
          expect(i32[1588 / 4]).to.equal(1); // this is good
          expect(i32[1592 / 4]).to.equal(3);

          let func = instance.exports.__indirect_function_table.get(1);
