...
let print_wasm_tree m =
  ...
  let stack_type = function
    | [] -> "(empty type)"
    | [t] -> value_type t
    | _ -> failwith "cannot encode stack type with arity > 1 (yet)"
  in
  ...
  let dump_types ts =
    let func_type = function
      | FuncType (ins, out) ->
          sexp "func"
            ( List.map (fun t -> spf "(param %s)" t) (List.map value_type ins)
            @ List.map (fun t -> spf "(result %s)" t) (List.map value_type out)
            )
    in
    func_type ts
  in
 ...
 let func_index symbol =
  ...
  let rec instr = function
    ...
    | Nop -> "nop"
    | Br x -> spf "br %ld" x
    | BrIf x -> spf "br_if %ld" x
    | BrTable (xs, x) ->
        Int32.(
          "br_table " ^ String.concat " " (List.map to_string xs) ^ to_string x)
    | Return -> "return"
    | Call symbol -> spf "call %ld" (func_index symbol)
    | CallIndirect x -> spf "call_indirect (type %ld)" (find_type x)
    | Drop -> "drop"
    | Select -> "select"
    ...
    | Block (ts, es) ->
        spf "block \n%s\n%s\n end" (result_type ts)
          (String.concat "\n"
             (List.map (fun x -> spf "(%s)" x) (List.map instr es)))
    | Loop (ts, es) ->
        spf "loop \n%s\n%s\n end" (result_type ts)
          (String.concat "\n"
             (List.map (fun x -> spf "(%s)" x) (List.map instr es)))
    | If (ts, es1, es2) ->
        spf "if %s \n %s \n else %s end" (result_type ts)
          (String.concat "\n"
...
let encode m =
  if !dump_wasmtree then print_wasm_tree m ;
  let s = stream () in
  ...
  let module E = struct
    ...
    let elem_type = function AnyFuncType -> vs7 (-0x10)
    ...
    let reloc_index x = vu32_fixed x
    ...
    let func_symbol_index symbol =
      let rec f symbol symbols result =
        ...
        | {name; details= Function} :: remaining
         |{name; details= Import _} :: remaining
          when name = symbol ->
            result
    ...
    let rec instr e =
      ...
      | Call symbol ->
          ...
          code_relocations :=
            !code_relocations
            @ [R_WEBASSEMBLY_FUNCTION_INDEX_LEB (Int32.of_int p, symbol)] ;
          reloc_index index
      | CallIndirect x ->
          op 0x11 ;
          (* (match !temp with
        ...
        temp := None; *)
          ...
          code_relocations :=
            !code_relocations
            @ [R_WEBASSEMBLY_TYPE_INDEX_LEB (Int32.of_int p2, x)] ;
          reloc_index x ;
          u8 0x00
      | Drop -> op 0x1a
      | Select -> op 0x1b
      ...
      | GetGlobal x ->
          ...
          code_relocations :=
            !code_relocations
            @ [R_WEBASSEMBLY_GLOBAL_INDEX_LEB (Int32.of_int p, x)] ;
          reloc_index 3l
      | SetGlobal x ->
          op 0x24 ;
          let p = pos s in
      ...
      | FuncSymbol symbol ->
          ...
          List.iteri
            (fun symbol_index s ->
              ...
              | (Import _ | Function) when s.name = symbol && not !found ->
                  found := true ;
                  code_relocations :=
                    !code_relocations
                    @ [R_WEBASSEMBLY_TABLE_INDEX_SLEB (Int32.of_int p, symbol)] ;
          ...
          List.iteri
            (fun symbol_index s ->
              ...
              | Data {index; offset} when s.name = symbol && not !found ->
                  ...
                  code_relocations :=
                    !code_relocations
                    @ [R_WEBASSEMBLY_MEMORY_ADDR_SLEB (Int32.of_int p, symbol)] ;
                  if
                    symbol = "caml_globals_inited"
                    || symbol = "caml_backtrace_pos"
                    || index = -1l
                  then vs32_fixed offset
                  else vs32_fixed (Int32.add offset 4l)
              | (Import _ | Function) when s.name = symbol && not !found ->
                  found := true ;
                  code_relocations :=
                    !code_relocations
    ...
    let code f =
      ...
      let remove_first locals no_of_removals =
        let rec rem counter locals =
          if counter < no_of_removals then rem (counter + 1) (List.tl locals)
          else locals
        in
        rem 0 locals
      in
    ...
    let table_segment seg = segment (vec reloc_index) seg
          ...
          | Symbol symbol ->
              ...
              List.iteri
                (fun symbol_index s ->
                  ...
                  | Data {index; offset} when s.name = symbol ->
                      ...
                      data_relocations :=
                        ...
                        @ [ R_WEBASSEMBLY_MEMORY_ADDR_I32
                              (Int32.of_int p, symbol) ] ;
                      if offset = -1l then u32 0l else u32 offset
                  | (Function | Import _) when s.name = symbol ->
                      found := true ;
                  ...
                  | _ -> () )
                m.symbols ;
              if not !found then failwith ("Not found symbol: " ^ symbol)
          | FunctionLoc symbol ->
              let p = pos s in
              let symbol_index = func_index symbol in
              data_relocations :=
                !data_relocations
                @ [R_WEBASSEMBLY_TABLE_INDEX_I32 (Int32.of_int p, symbol)] ;
              u32 symbol_index
          | Int32 i32 -> u32 i32
        ...
        (fun i r ->
              ...
              List.iteri
                (fun symbol_index s ->
                  ...
                  | Data {index} when s.name = symbol_ ->
                      ...
                      if
                        symbol_ = "caml_globals_inited"
                        || symbol_ = "caml_backtrace_pos"
                      then vs32 0l
                      else vs32 4l
                  | (Import _ | Function) when s.name = symbol_ ->
                      exists := true ;
                      u8 1 ;
                      vu32 (Int32.sub offset !code_pos) ;
                      vs32_fixed (Int32.of_int symbol_index)
          ...
          | R_WEBASSEMBLY_MEMORY_ADDR_LEB (offset, index_) ->
              ...
              List.iteri
                (fun i s ->
                  match s.details with
                  | Data {index; _} when index = index_ -> symbol_index := i
                  | _ -> () )
                m.symbols ;
              u8 3 ;
              vu32 (Int32.sub offset !code_pos) ;
        ...
        (fun r ->
          ...
          | R_WEBASSEMBLY_MEMORY_ADDR_I32 (offset, symbol_) ->
              ...
              List.iteri
                (fun i s ->
                  match s.details with
                  | Data _ when s.name = symbol_ -> symbol_index := i
                  | _ -> () )
                m.symbols ;
    ...
    let symbol sym =
      ...
      | Import _ | Function -> u8 0
      | Data _ -> u8 1
      | Global _ -> u8 2 ) ;
      let flags = ref (match sym.details with Import _ -> 0l | _ -> 1l) in
      let exists =
        match sym.details with
        | Function ->
            if
            ...
            then
              failwith
                ( "BUG: symbol " ^ sym.name
                ^ " appears to refer to a nonexisting function, perhaps it \
                   should refer to an import instead?" ) ;
            List.exists (fun (f : Ast.func) -> f.name = sym.name) m.funcs
        | Import _ -> false
        | Data _ -> data_index sym.name <> -1l
        | Global _ -> true
      in
      if not exists then flags := Int32.logor !flags 16l ;
      ( match sym.details with
      ...
      | Global f ->
          vu32 f.index ;
          if exists then string sym.name
      | Function ->
          vu32 (func_index sym.name) ;
          if exists then string sym.name
      | Import _ -> vu32 (func_index sym.name)
      | Data d ->
          if sym.name <> "" then string sym.name else string "_" ;
    ...
    let symbol_table (data2 : data_part segment list) =
      ...
      List.iter
        (fun (d : Ast.data_part Ast.segment) ->
          string d.init.name ; vu32 4l ; vu32 0l )
        data2 ;
      patch_gap32 g (pos s - p) ;
      u8 8 ;
      (* WASM_SYMBOL_TABLE *)
    ...
    let module_ m =
      ...
      if List.length !code_relocations > 0 then relocate_code_section m.funcs ;
      if List.length !data_relocations > 0 then relocate_data_section m.funcs ;
      name_section m
  end in
  E.module_ m ;
