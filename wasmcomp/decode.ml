...
let error s pos msg = ignore(s); ignore(pos); failwith msg
 (* raise (Code (region s pos pos, msg)) *)
...
let rec instr s =
  ...
  | 0x06| 0x07 | 0x08 | 0x09 | 0x0a as b -> ignore(b); nop
   (* illegal s pos b *)
  ...
  | 0x20 -> get_local (var s)
  | 0x21 -> set_local (var s)
  | 0x22 -> tee_local (var s)
  | 0x23 -> get_global (var s)
  | 0x24 -> set_global (var s)
  ...
  | 0x41 -> i32_const (vs32 s)
  | 0x42 -> i64_const (vs64 s)
  | 0x43 -> f32_const (f32 s)
  | 0x44 -> f64_const (f64 s)
...
let import s =
  ...
  let idesc = import_desc s in
  {module_name; item_name; idesc}

let import_section s =
  section `ImportSection (vec import) [] s


(* Function section *)

let func_section s =
  section `FuncSection (vec var) [] s
...
let table_section s =
  section `TableSection (vec table) [] s
...
let memory_section s =
  section `MemorySection (vec memory) [] s
...
let global_section s =
  section `GlobalSection (vec global) [] s
...
let export_desc s =
  match u8 s with
  | 0x00 -> FuncExport (var s)
  | 0x01 -> TableExport (var s)
  | 0x02 -> MemoryExport (var s)
  | 0x03 -> GlobalExport (var s)
  | _ -> error s (pos s - 1) "invalid export kind"

let export s =
  let name = name s in
  let edesc = export_desc s in
  {name; edesc}

let export_section s =
  section `ExportSection (vec export) [] s


(* Start section *)

let start_section s =
  section `StartSection (opt var true) None s
...
let code _ s =
  ...
  end_ s;
  {name = ""; locals; body; ftype = (-1l)}

let code_section s =
  section `CodeSection (vec (sized code)) [] s
...
let elem_section s =
  section `ElemSection (vec table_segment) [] s

(* Data section *)
let data_part_list s =
  [String (string s)]

let memory_segment s =
  segment data_part_list s

let data_section s =
  section `DataSection (vec memory_segment) [] s
...
let module_ s =
  ...
  let funcs =
    List.map2 (fun t f -> {f with ftype = t})
