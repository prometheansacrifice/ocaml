...
let mk_annot f = ("-annot", Arg.Unit f, " Save information in <filename>.annot")
...
let mk_o3 f =
  ( "-O3"
  , Arg.Unit f
  , " Apply aggressive optimization for speed (may significantly increase \
...
let mk_classic_inlining f =
  ...
  , " Make inlining decisions at function definition time rather than at the \
     call site (replicates previous behaviour of the compiler)" )
...
let mk_plugin f =
  ("-plugin", Arg.String f, "<plugin>  Load dynamic plugin <plugin>")
...
let mk_unsafe_string f =
  ...
  else ("-unsafe-string", Arg.Unit f, " Make strings mutable (default)")
...
let mk_dflambda f = ("-dflambda", Arg.Unit f, " Print Flambda terms")
...
let mk_dwasmtree f = ("-dwasmtree", Arg.Unit f, " displays internal wasm tree")
