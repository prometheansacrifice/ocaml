...
module Backend = struct
  ...
  let max_sensible_number_of_arguments = 10
end
...
module Options = Main_args.Make_optcomp_options (struct
  ...
  *)
  ...
  let _color option =
    match parse_color_setting option with
    | None -> ()
  ...
  let _dwasmtree = set dump_wasmtree
  ...
  try
    ...
    if
      List.length
        (List.filter
           (fun x -> !x)
           [make_package; make_archive; shared; compile_only; output_c_object])
      > 1
    then
